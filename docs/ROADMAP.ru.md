...
Настройка для чата когда бот должен отвечать:
- **Все сообщения** — отвечать на всё (текущее поведение)
- **Пропускать упоминания** — не отвечать когда упомянут кто-то другой
- **Только упоминание бота** — отвечать только когда упомянут бот
- Переключение через `/settings` или inline кнопки
