...
### Скачивание файлов от Claude
Скачивание файлов, созданных Claude в процессе работы:
- Детектить уведомления о создании файлов в выводе Claude
- Предлагать кнопку скачивания в Telegram
- Поддержка типичных файлов (код, конфиги, картинки)
- Авто-определение новых файлов в директории проекта
...
### Безопасный контекст чата
Подкидывать безопасный контекст из истории чата в Claude:
- Извлекать релевантный контекст из диалога
- Санитизировать чувствительные данные
- Форматировать как структурированный контекст

### Изолированная безопасная среда
Создание sandbox окружения для рискованных операций:
- Docker/container-based изоляция
- Отдельная файловая система для экспериментов
- Простая очистка после сессии

### Кнопки с подсказками / inline suggests
Быстрые кнопки действий и подсказки в чате:
- Context-aware кнопки предложений
- Частые команды как inline buttons
- Подсказки на основе текущего состояния сессии
