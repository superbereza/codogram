...
### Настройка скрыть/показать thinking
Переключение видимости блоков `<thinking>` от Claude:
- Per-chat настройка
- По умолчанию скрыто для чистоты вывода
- Показывать для дебага или обучения

### Сворачиваемые permission prompts
По умолчанию только заголовок, раскрытие по запросу:
- Permission prompt показывает только действие (Bash, Read, Edit и т.д.)
- Кнопка `[Show more]` раскрывает полный контекст
- Сообщение редактируется на месте
- Меньше шума, но вся информация доступна
...
Более гранулярный контроль над verbose output:
- `/verbose` открывает подменю с текущей настройкой
- `[full]` — показывать всё (текущее поведение verbose on)
- `[-5 strings] [+5 strings]` — настройка лимита строк
- `[just headers]` — только названия тулов (как 0 строк)
- `[only current header]` — одно сообщение, обновляется с новыми хедерами
- `[total silence]` — только сообщения юзеру
- `[• on/off]` — вкл/выкл жирную точку в начале
- `[save settings]` — сохранить выбор

### Глобальные настройки в личке
Команда `/settings` в личке с ботом для дефолтов всех чатов:
- Дефолтный verbose mode, response mode, auto-accept и т.д.
- Новые чаты/треды наследуют эти дефолты
- Per-chat настройки переопределяют глобальные
- Удобный способ настроить один раз
